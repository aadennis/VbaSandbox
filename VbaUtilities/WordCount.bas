...
' Assume that ":" is the passed delimiter)
' Given consecutive tokens in a line, look for the token ":" (assumption that there will be only once such token/delimiter
' in a line). Now, find and save the preceding token and the following token (relative to the delimiter
' token, skipping any whitespace. (if none found return).
' If the two tokens have the same value, then return True, else return False
...
' immediately followed by the passed delimiter (once any white space is ignored), followed a second instance of the
' same token value.
